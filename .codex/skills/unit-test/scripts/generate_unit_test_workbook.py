...
def _copy_row_style(
    ...
    for col in range(1, ws.max_column + 1):
        ...
        if source_cell._comment is not None:
...
def _insert_styled_rows(
    ...
    for offset in range(amount):
        _copy_row_style(ws, style_source_row + amount, start_row + offset)
...
def _populate_function_sheet(
    ...
    # Expand the template dynamically when the requested UTC matrix needs more
    # condition/confirm rows than the stock function sheet provides.
    ...
    if condition_required > condition_capacity:
        extra_condition_rows = condition_required - condition_capacity
        _insert_styled_rows(
            ws,
            layout.confirm_row,
            extra_condition_rows,
            max(layout.condition_row + 1, layout.confirm_row - 1),
        )
        layout = _detect_layout(ws.parent, ws.title)
    ...
    if confirm_required > confirm_capacity:
        extra_confirm_rows = confirm_required - confirm_capacity
        _insert_styled_rows(
            ws,
            layout.result_row,
            extra_confirm_rows,
            max(layout.confirm_row + 1, layout.result_row - 1),
        )
        layout = _detect_layout(ws.parent, ws.title)
