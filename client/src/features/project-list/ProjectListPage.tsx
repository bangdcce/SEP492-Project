...
export default function ProjectListPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await fetchProjectsByUser(MOCK_USER_ID);
        setProjects(data);
      } catch (err: any) {
  ...
  const renderRole = (project: Project) => {
    if (project.freelancerId === MOCK_USER_ID) return "Freelancer";
    if (project.clientId === MOCK_USER_ID) return "Client";
    return "Collaborator";
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Select a project to open its workspace.</p>
        </div>
      </div>
