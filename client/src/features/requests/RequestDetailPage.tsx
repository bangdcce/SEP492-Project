...
export default function RequestDetailPage() {
  ...
  const fetchData = async (requestId: string) => {
    try {
      ...
      if (
        reqData &&
        (reqData.status === "HIRING" || reqData.status === "IN_PROGRESS")
      ) {
  ...
  const handleAcceptSpecs = () => {
    setSpecsAccepted(true);
    // Optimistic update
    setRequest((prev: typeof request) => ({ ...prev, status: "HIRING" }));
    ...
    <div className="container mx-auto p-4 md:p-6 max-w-7xl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
        ...
        {request.status === "DRAFT" && (
          <Button onClick={() => navigate(`/wizard?draftId=${request.id}`)}>
            Edit Draft
          </Button>
        )}
      </div>
              ...
              <Card>
                ...
                <CardContent>
                  {currentPhase < 3 ? (
                    <div className="text-center py-12 bg-muted/20 border-2 border-dashed rounded-lg">
                      <p className="text-muted-foreground">
                        Recruitment will be available after Spec Approval.
                      </p>
                    </div>
                  ) : matches.length === 0 ? (
                        ...
                        >
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              size="sm"
                              onClick={() =>
                                handleInvite(match.broker.fullName)
                              }
                            >
