      ...
      <CardContent className="flex-1 flex flex-col pt-4 gap-4 h-[400px]">
        <div className="flex-1 overflow-y-auto space-y-4 pr-2">
            {comments.map((comment) => (
                <div key={comment.id} className={`flex gap-3 ${comment.isMe ? 'flex-row-reverse' : ''}`}>
                    <Avatar className="w-8 h-8">
                        <AvatarFallback>{comment.user.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div className={`max-w-[80%] rounded-lg p-3 text-sm ${comment.isMe ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                        <p>{comment.text}</p>
