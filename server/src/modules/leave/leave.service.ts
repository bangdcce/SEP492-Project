...
import {
  buildLeaveAvailabilityDaySlots,
...
export class LeaveService {
  ...
  async getApprovedLeaveStaffIdsAt(referenceAt: Date, staffIds: string[]): Promise<Set<string>> {
    ...
    const requests = await this.leaveRequestRepository.find({
      where: {
        staffId: In(staffIds),
        status: LeaveStatus.APPROVED,
        startTime: LessThan(referenceAt),
        endTime: MoreThan(referenceAt),
      },
    });

    return new Set(requests.map((request) => request.staffId));
  }

  async getApprovedLeaveStaffIdsOverlapping(
    rangeStart: Date,
    rangeEnd: Date,
    staffIds: string[],
  ): Promise<Set<string>> {
    if (staffIds.length === 0 || rangeStart >= rangeEnd) {
      return new Set();
    }
  ...
  private async applyLeaveToAvailability(request: StaffLeaveRequestEntity, timeZone: string) {
    const slots = buildLeaveAvailabilityDaySlots(request.startTime, request.endTime, timeZone);
  ...
  private async attachActorSummaries(items: StaffLeaveRequestEntity[]) {
    const userIds = items.flatMap((item) => [
      ...
      ...(item.cancelledById ? [item.cancelledById] : []),
    ]);
    ...
    return items.map((item) => ({
      ...
      cancelledBy: item.cancelledById ? (summaryMap.get(item.cancelledById) ?? null) : null,
    }));
  }

  private async attachProcessedBySummaries(items: StaffLeaveRequestEntity[]) {
    const actorIds = items
      .flatMap((item) => [item.processedById, item.cancelledById])
      .filter((id): id is string => Boolean(id));
    const summaryMap = await this.mapUserSummaries(actorIds);
