...
describe('LeaveService', () => {
  ...
  it('returns approval and cancellation summaries for cancelled leave request listing', async () => {
    const now = new Date();
    const listQb = {
      andWhere: jest.fn().mockReturnThis(),
      orderBy: jest.fn().mockReturnThis(),
      getMany: jest.fn().mockResolvedValue([
        {
          ...
          isAutoApproved: false,
          processedAt: now,
          cancelledAt: new Date(now.getTime() + 10 * 60 * 1000),
          createdAt: now,
          updatedAt: now,
        },
      ] as StaffLeaveRequestEntity[]),
    ...
    expect(first.cancelledBy).toEqual({
      id: 'staff-1',
      fullName: 'Staff One',
      email: 'staff1@example.com',
    });
  ...
  it('records long-term leave outside business hours with zero quota minutes', async () => {
    ...
    leavePolicyRepository.findOne.mockResolvedValue({
      id: 'policy-1',
      staffId: 'staff-1',
      monthlyAllowanceMinutes: DEFAULT_MONTHLY_ALLOWANCE_MINUTES,
    });
    leaveRequestRepository.create.mockImplementation((input) => input);
    leaveRequestRepository.save.mockImplementation(async (input) => ({
      id: 'leave-1',
      ...input,
    }));
    ...
    const result = await service.createLeaveRequest(
      {
        type: LeaveType.LONG_TERM,
        startTime: '2099-01-12T20:00:00.000Z',
        endTime: '2099-01-12T22:00:00.000Z',
        reason: 'Overnight leave',
      },
      requester,
    );
  ...
  it('applies approved leave as blocking availability across the full leave range', async () => {
    const startTime = new Date('2099-01-12T20:00:00.000Z');
    const endTime = new Date('2099-01-13T06:00:00.000Z');
    ...
    leaveRequestRepository.createQueryBuilder.mockReturnValue(createUpdateQueryBuilderMock(1));
    availabilityService.setUserAvailability.mockResolvedValue(undefined);
    jest.spyOn(service as any, 'refreshLeavePerformanceForRequest').mockResolvedValue(undefined);

    const admin = { id: 'admin-1', role: UserRole.ADMIN } as UserEntity;
    const result = await service.processLeaveRequest('leave-1', { action: 'approve' }, admin);

    expect(result).toEqual({
      success: true,
      message: 'Leave request approved',
    });
    expect(availabilityService.setUserAvailability).toHaveBeenCalledWith(
      expect.objectContaining({
        userId: 'staff-1',
        allowConflicts: true,
        timeZone: 'UTC',
        ...
        skipExistingOverlapValidation: true,
        skipWorkingHoursValidation: true,
        slots: expect.arrayContaining([
          expect.objectContaining({
            startTime: '2099-01-12T20:00',
            endTime: '2099-01-12T23:59:59',
          }),
          expect.objectContaining({
            startTime: '2099-01-13T00:00',
            endTime: '2099-01-13T06:00',
          }),
        ]),
      }),
    );
