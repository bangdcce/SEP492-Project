...
export class AddTaskColumnJira1769155481889 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE "projects" SET "status" = 'PLANNING' WHERE "status" = 'INITIALIZING'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TYPE "public"."project_specs_status_enum" RENAME TO "project_specs_status_enum_old"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "project_specs" ALTER COLUMN "status" TYPE "public"."project_specs_status_enum" USING "status"::"text"::"public"."project_specs_status_enum"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."milestones_status_enum" AS ENUM('PENDING', 'IN_PROGRESS', 'LOCKED', 'COMPLETED', 'PAID')`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "tasks" ADD CONSTRAINT "FK_7ecc6be7d74a3f441f7aa5215ef" FOREIGN KEY ("reporterId") REFERENCES "users"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."milestones_status_enum_old" AS ENUM('PENDING', 'IN_PROGRESS', 'LOCKED', 'COMPLETED', 'PAID', 'SUBMITTED', 'REVISIONS_REQUIRED')`,
    );
    await queryRunner.query(`ALTER TABLE "milestones" ALTER COLUMN "status" DROP DEFAULT`);
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."project_specs_status_enum_old" AS ENUM('DRAFT', 'PENDING_APPROVAL', 'APPROVED', 'REJECTED', 'PENDING_AUDIT')`,
    );
    await queryRunner.query(`ALTER TABLE "project_specs" ALTER COLUMN "status" DROP DEFAULT`);
    await queryRunner.query(
      `ALTER TABLE "project_specs" ALTER COLUMN "status" TYPE "public"."project_specs_status_enum_old" USING "status"::"text"::"public"."project_specs_status_enum_old"`,
    );
    await queryRunner.query(
      `ALTER TABLE "project_specs" ALTER COLUMN "status" SET DEFAULT 'PENDING_APPROVAL'`,
    );
    ...
    await queryRunner.query(
      `ALTER TYPE "public"."project_specs_status_enum_old" RENAME TO "project_specs_status_enum"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."projects_status_enum_old" AS ENUM('PLANNING', 'IN_PROGRESS', 'TESTING', 'COMPLETED', 'PAID', 'DISPUTED', 'CANCELED', 'INITIALIZING')`,
    );
    await queryRunner.query(`ALTER TABLE "projects" ALTER COLUMN "status" DROP DEFAULT`);
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "milestones" ADD "retentionAmount" numeric(14,2) DEFAULT '0'`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."deliverable_type_enum" AS ENUM('DESIGN_PROTOTYPE', 'API_DOCS', 'DEPLOYMENT', 'SOURCE_CODE', 'SYS_OPERATION_DOCS', 'CREDENTIAL_VAULT', 'OTHER')`,
    );
    await queryRunner.query(
      `ALTER TABLE "milestones" ADD "deliverableType" "public"."deliverable_type_enum" DEFAULT 'OTHER'`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "users" ADD "emailVerificationToken" character varying(64)`,
    );
    await queryRunner.query(
