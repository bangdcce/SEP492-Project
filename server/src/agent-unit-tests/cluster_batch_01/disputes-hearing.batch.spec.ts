...
const endpoints = loadDisputesEndpoints('hearing-controller');

describe('Disputes cluster batch - hearing controller', () => {
  jest.setTimeout(300000);
  let runtime: Awaited<ReturnType<typeof createDisputesGroupRuntime>>;

  beforeAll(async () => {
    runtime = await createDisputesGroupRuntime('hearing-controller');
  });
