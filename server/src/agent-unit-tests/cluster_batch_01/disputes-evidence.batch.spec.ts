...
const endpoints = loadDisputesEndpoints('evidence-controller');

describe('Disputes cluster batch - evidence controller', () => {
  jest.setTimeout(300000);
  let runtime: Awaited<ReturnType<typeof createDisputesGroupRuntime>>;

  beforeAll(async () => {
    runtime = await createDisputesGroupRuntime('evidence-controller');
  });
