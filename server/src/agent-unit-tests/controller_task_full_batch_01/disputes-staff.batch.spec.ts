...
const endpoints = loadDisputesEndpoints('staff-assignment-controller');

describe('Disputes cluster batch - staff assignment controller', () => {
  jest.setTimeout(300000);
  let runtime: Awaited<ReturnType<typeof createDisputesGroupRuntime>>;

  beforeAll(async () => {
    runtime = await createDisputesGroupRuntime('staff-assignment-controller');
  });
