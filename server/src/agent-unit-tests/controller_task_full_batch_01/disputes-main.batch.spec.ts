...
const endpoints = loadDisputesEndpoints('disputes-controller');

describe('Disputes cluster batch - main controller', () => {
  jest.setTimeout(300000);
  let runtime: Awaited<ReturnType<typeof createDisputesGroupRuntime>>;

  beforeAll(async () => {
    runtime = await createDisputesGroupRuntime('disputes-controller');
  });
